...
script.onload = () => {
    createUnityInstance( document.getElementById('unity-canvas'), config, (progress) => {
        ...
        document.getElementById('unity-fullscreen-button').onclick = () => {
            unityInstance.SendMessage('EyelashMakerMirror', 'MakeEyelash', eyelashjson)
            unityInstance.SendMessage('EyelashMakerMirror (1)', 'MakeEyelash', eyelashjson)
            unityInstance.SendMessage('DynamicAvatar1', 'SetFaceParameter', facejson)
        };
    }).catch((message) => {
        alert(message);
    });
